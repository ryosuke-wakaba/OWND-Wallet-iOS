...
enum JWTUtil {
    ...
    /// Verify JWT by x5c header with TrustedList support
    /// - Parameters:
    ///   - jwt: The JWT string to verify
    ///   - issuerURL: Optional issuer URL to lookup certificates from TrustedList
    ///   - verifyCertChain: Whether to verify the certificate chain
    /// - Returns: Result with verified JWT and certificates, or error
    static func verifyJwtByX5C(
        ...
        do {
            ...
            print("🔐 [JWTUtil] Failed to convert x5c to certificates: \(error.localizedDescription)")
            return .failure(.verificationFailed(error.localizedDescription))
        } catch {
            print("🔐 [JWTUtil] Failed to convert x5c to certificates: \(error)")
            return .failure(.verificationFailed("Unable to convert x5c: \(error.localizedDescription)"))
        }
        print("🔐 [JWTUtil] Certificates converted: \(certificates.count)")
        ...
        guard
            ...
            print("🔐 [JWTUtil] Failed to create SecKey: \(error?.takeRetainedValue().localizedDescription ?? "unknown")")
            return .failure(.verificationFailed("Unable to Convert Public Key"))
        }
        print("🔐 [JWTUtil] SecKey created successfully")

        let jwtValidation = JWTUtil.verifyJwt(jwt: jwt, publicKey: secKey)
        print("🔐 [JWTUtil] JWT signature validation result: \(jwtValidation)")

        if case .success = jwtValidation {
            if verifyCertChain {
                // Convert X509.Certificate to SecCertificate
                ...
                guard secCertificates.count == certificates.count else {
                    print("🔐 [JWTUtil] Failed to convert all certificates to SecCertificate")
                    return .failure(.verificationFailed("Unable to convert certificates"))
                }

                // Determine which TrustAnchorManager to use
                let trustAnchorManager: TrustAnchorManager
                if let issuerURL = issuerURL {
                    print("🔐 [JWTUtil] ========== Using TrustedList for issuer: \(issuerURL) ==========")
                    do {
                        ...
                        print("🔐 [JWTUtil] ⚠️ TrustedList lookup failed: \(error). Using singleton.")
                        trustAnchorManager = TrustAnchorManager.shared
                    }
                } else {
                    print("🔐 [JWTUtil] No issuerURL provided, using singleton TrustAnchorManager")
                    trustAnchorManager = TrustAnchorManager.shared
                }

                // Validate certificate chain with the selected TrustAnchorManager
                ...
                case .success:
                    print("🔐 [JWTUtil] Certificate chain validation succeeded")
                case .failure(let certError):
                    print("🔐 [JWTUtil] Certificate chain validation failed: \(certError.errorDescription ?? "unknown")")
                    return .failure(.certificateValidationFailed(certError))
                }
            } else {
                print("🔐 [JWTUtil] Skip ValidateCertificateChain!!!")
            }
