...
enum JWTOperations {
    ...
    private struct ECDSASignature: DERImplicitlyTaggable {
    ...
    private static func convertRstoDer(r: Data, s: Data) -> Data? {
        let rArray = [UInt8](r)[...]
        let sArray = [UInt8](s)[...]
    ...
    private static func getAlgorithm(publicKey: SecKey, algorithm: String) -> SecKeyAlgorithm? {
        ...
        -> Result<String, SigningError>
    {
        ...
        guard
            ...
            return .failure(.unableToCreateSignature)
        }
    ...
    static func verifyJwt(jwt: String, publicKey: SecKey) -> Result<JWT, VerificationError> {
        ...
        if result {
            ...
            return .failure(.verificationFailed("result of SecKeyVerifySignature is false"))
        }
    }
    ...
    static func decodeJwtWithX5C(jwt: String) -> Result<(decoded: JWT, x5c: [String]), VerificationError> {
        guard let decodedJwt = try? decode(jwt: jwt) else {
            print("🔐 [JWTOperations] Failed to decode JWT")
            return .failure(.verificationFailed("Unable to decode jwt"))
        }
        print("🔐 [JWTOperations] JWT decoded, header keys: \(decodedJwt.header.keys)")

        guard let x5c = decodedJwt.header["x5c"] as? [String] else {
            print("🔐 [JWTOperations] x5c not found in header")
            return .failure(.verificationFailed("Unable to get x5c property"))
        }
        print("🔐 [JWTOperations] x5c found, count: \(x5c.count)")
    ...
    static func decodeJwtWithX5U(jwt: String) -> Result<(decoded: JWT, x5uUrl: String), VerificationError> {
        guard let decodedJwt = try? decode(jwt: jwt) else {
            return .failure(.verificationFailed("Unable to decode jwt"))
        }
        ...
        print("🔐 [JWTOperations] x5u url: \(x5uUrl)")
